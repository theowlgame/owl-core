...
contract OWL is Ownable, Pausable, IERC20 {
    ...
    uint256 private constant _tTotal = 500_000_000 * 10**18; // 500 M
    uint256 private _rTotal; // (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "OwlDAO token"; // CKP-12
    ...
    uint256 public _maxTxAmount; // 1_000_000_000 * 10**18;       // 1 B
    uint256 private numTokensSellToAddToLiquidity; // 10_000_000 * 10**18;   // 10 M
    ...
    constructor(address _router, address _usdToken, ITokenSwap _TokenSwap) public {

        //_name = "OwlDAO token";
        //_symbol = "OWL";
        //_decimals = 18;

        //_tTotal = totalSupply * 10**18; // 500 M
        ...
        _maxTxAmount = _tTotal; // 500 M  CKP-09
        numTokensSellToAddToLiquidity = 10_000_000 * 10**18;   // 10 M
    ...
    function includeInReward(address account) external onlyOwner() {
        require(_isExcluded[account], "Account already included");      // CKP-07
        for (uint256 i = 0; i < _excluded.length; i++) {
            if (_excluded[i] == account) {
                _excluded[i] = _excluded[_excluded.length - 1];
                _rOwned[account] = _tOwned[account].mul(_getRate());    // CKP-05: recalculate _rOwned
                _tOwned[account] = 0;
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        require(maxTxPercent > 0);  // CKP-06
        _maxTxAmount = _tTotal.mul(maxTxPercent).div(10**2);
    }

    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount > 0);
    ...
    function swapTokensForToken(uint256 tokenAmount) private {
        ...
        // generate the uniswap pair path of token -> USDToken
    ...
    function setTokenSwap(ITokenSwap _TokenSwap) external onlyOwner {
        require(address(_TokenSwap) != address(0));
        TokenSwap = _TokenSwap;
    }
}
